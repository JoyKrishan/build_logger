...
def extract_build_links(comments):
    ...
    for comment in comments:
        ...
        tag= comment.get('tag', '')
        # Checks if the comment is by Zuul
        if len(tag.split(':')) > 1 and tag.split(':')[1] == 'zuul' and author_username and author_username.lower() == 'zuul':
            lines = comment_text.split('\n')
            for line in lines:
                if line.startswith('- '): 
                    line = line.strip().split(' ')
                    if len(line) == 8 and line[2].startswith('https://zuul.opendev.org/t/openstack/build/'):
                        # TODO: Track all info (i.e., job_name, outcome, build_time) in the build link with a CSV file
                        build_links.append({
...
def collect_all_log_files(zuul_uuid):
    zuul_api_url = f"{ZUUL_API}/build/{zuul_uuid}"
    try:
        ...
        if e.code == 404:
            print("Could not find build UUID in Zuul API. This can happen with ", file=sys.stderr)
        else:
...
def save_log_files(file_path, base_url, save_dir):
    ...
    result = subprocess.run(cmd, cwd=save_dir)
    ...
    if change_details:
        ...
        if comments:
            ...
            for build_info in tqdm(all_build_links, desc="Processing builds", position=0, leave=True, colour='green'):
                ...
                zuul_uuid = build_info['url'].split('/')[-1]
                log_files = collect_all_log_files(zuul_uuid)
                
                save_dir = os.path.join(download_path, comment_id, 'builds', zuul_uuid)
                os.makedirs(save_dir, exist_ok=True)
                
                base_log_url = log_files[0]
                for log_file in tqdm(log_files[1:], desc=f"Downloading logs for build:{zuul_uuid}", position=1, leave=False):
